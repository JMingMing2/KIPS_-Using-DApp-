...
contract VM {
	...
	// 취약점 등록
	function add_vuln(string v_name , string v_link , uint v_p_id , uint v_c_id, uint v_cvss, v_poc) returns (uint vl_id) 
	{
		if(Companys[v_c_id].c_addr == msg.sender)
		{
			...
			{
				vl_id = vl_no++;
				Vulns[vl_id] = Vuln(vl_id, v_name , v_link, v_p_id, v_c_id, v_cvss, v_poc);
			}	
		}
	}
